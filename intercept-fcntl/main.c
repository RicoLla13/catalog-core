...
int main(int argc, char *argv[])
{
	...
	if (fdflags >= 0) {
		dprintf(1, OK_PREFIX " fcntl(%d, F_GETFD) succeeded: flags=%d errno=%d\n",
			fd, fdflags, errno);
	} else {
		dprintf(1, ERR_PREFIX " fcntl(%d, F_GETFD) failed: rc=%d errno=%d (%s)\n",
	...
	if (rc == 0) {
		dprintf(1, OK_PREFIX " fcntl(%d, F_SETFD, FD_CLOEXEC) succeeded: rc=%d errno=%d\n",
			fd, rc, errno);
	} else {
		dprintf(1, ERR_PREFIX " fcntl(%d, F_SETFD, FD_CLOEXEC) failed: rc=%d errno=%d (%s)\n",
	...
	if (dupfd >= 10) {
		dprintf(1, OK_PREFIX " fcntl(%d, F_DUPFD, 10) succeeded: newfd=%d errno=%d\n",
			fd, dupfd, errno);
	} else {
		dprintf(1, ERR_PREFIX " fcntl(%d, F_DUPFD, 10) failed: rc=%d errno=%d (%s)\n",
	...
	if (rc == 0) {
		dprintf(1, OK_PREFIX " fcntl(%d, F_SETLK, WRLCK) succeeded: rc=%d errno=%d\n",
			fd, rc, errno);
	} else {
		dprintf(1, ERR_PREFIX " fcntl(%d, F_SETLK, WRLCK) failed: rc=%d errno=%d (%s)\n",
			fd, rc, errno, strerror(errno));
		failed = 1;
		goto out;
	}
	...
	rc = fcntl(fd, F_SETLK, &flk);
	if (rc == 0) {
		dprintf(1, OK_PREFIX " fcntl(%d, F_SETLK, UNLCK) succeeded: rc=%d errno=%d\n",
			fd, rc, errno);
	} else {
		dprintf(1, ERR_PREFIX " fcntl(%d, F_SETLK, UNLCK) failed: rc=%d errno=%d (%s)\n",
			fd, rc, errno, strerror(errno));
		failed = 1;
		goto out;
	}

...
static int visit_entry(int rootfd, const char *entry_name, int *checked_pread)
{
	...
	printf(OK_PREFIX " fstat(%d) succeeded: rc=%d errno=%d size=%lld\n",
	...
	if (!*checked_pread) {
		...
		if (pos < 0) {
			printf(ERR_PREFIX " lseek(%d, 0, SEEK_CUR) failed: off=%lld errno=%d (%s)\n",
		...
		if (nread < 0) {
			printf(ERR_PREFIX " pread(%d, 15, 0) failed: rc=%zd errno=%d (%s)\n",
...
int main(int argc, char *argv[])
{
	...
	if (nread >= 0) {
		...
		printf(ERR_PREFIX " read(%d, %zu) manifest failed: rc=%zd errno=%d (%s)\n",
